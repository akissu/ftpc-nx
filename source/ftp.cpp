...
#define CONNECT_IP      "192.168.86.187"
//#define CONNECT_IP      "192.168.60.1"
...
static const size_t num_ftpc_commands = sizeof(ftpc_commands)/sizeof(ftpc_commands[0]);
...
static int                sock = -1;
static int                sock_pasv = -1;
static int                port_pasv = -1;
#ifdef _3DS
/*! current data port */
...
ftpc_init(void)
{
  ...
  simple_recv(sock, b);
  simple_send(sock, "USER anonymous\n", 15);
  simple_recv(sock, b);
  simple_send(sock, "PASS anonymous\n", 15);
  simple_recv(sock, b);

  port_pasv= pasv_create_port(sock, b);
  pasv_do_cmd(sock, port_pasv, b, "LIST\n", 5);
  simple_send(sock, "CWD Storage\n", 12);
  simple_recv(sock, b);
  /*
  port_pasv= pasv_create_port(sock, b);
  pasv_do_cmd(sock, port_pasv, b, "LIST\n", 5);
...
int simple_recv(int s, char * buffer) 
{
  ...
  int received = -1;
  int bytes = 0;
  console_print(GREEN "recv:start\n" RESET);
  bytes = recv(s, buffer, buffsize-1, 0);
  while(bytes > 0)
  {
    received += bytes;
    buffer[bytes] = '\0';        /* Assure null terminated string */
    console_print(WHITE "%s" RESET, buffer);
    if(bytes < buffsize-1)
      break;
    bytes = recv(s, buffer, buffsize-1, 0);
  }
  if(bytes < 1) 
  {
    console_print(RED "Failed to receive bytes from server\n" RESET);
    return -1;
  }
  console_print(GREEN "recv:end\n" RESET);
  return 0;
}
...
int simple_recv_code(int s, char * buffer) 
{
  ...
  received += bytes;
  buffer[bytes] = '\0';        /* Assure null terminated string */
  int start = strchr(buffer, '(');
  int end = strchr(buffer+start, ')');
  console_print(GREEN "recv: %s,%d,%d\n" RESET, buffer, start, end);
  return (int)buffer[0];
}
...
int simple_send(int s, char * cmd, int cmd_len)
{
  if (send(s, cmd, cmd_len, 0) != cmd_len) {
    console_print(RED "Mismatch in number of sent bytes\n" RESET);
    return -1;
  }
  return 0;
}

int pasv_recv(int s, char * buffer) 
{
  int buffsize = 64;
  int received = -1;
  int bytes = 0;
  ...
  received += bytes;
  buffer[bytes] = '\0';        /* Assure null terminated string */
  console_print(GREEN "recv: %s\n" RESET, buffer);
  return 0;
}

int pasv_create_port(int sock, char * b)
{
  simple_send(sock, "PASV\n", 5);
  pasv_recv(sock, b);
  console_print(GREEN "pasv_create_port recv: %s\n" RESET, b);
  int start = strchr(b, '(');
  ...
  start = strchr(start+1, ',');
  start++;
  int mid = strchr(start+1, ',');
  mid++;
  int end = strchr(mid+1, ')');
  console_print(GREEN "start: %d\n" RESET, start);
  console_print(GREEN "mid:   %d\n" RESET, mid);
  console_print(GREEN "end:   %d\n" RESET, end);
  char tmpb[6];
  memcpy(tmpb, start, mid-start);
  tmpb[end-start+1] = '\x00';
  int ss = atoi(tmpb);
  memcpy(tmpb, mid, end-start);
  tmpb[end-start+1] = '\x00';
  int xx = atoi(tmpb);
  console_print(GREEN "tmpb: %s\n" RESET, tmpb);
  console_print(GREEN "ss: %d\n" RESET, ss);
  console_print(GREEN "xx: %d\n" RESET, xx);
  console_print(GREEN "PASV Port: %d\n" RESET, ss*256+xx);
  return ss*256+xx;
}

int pasv_do_cmd(int sock, int pasv_port, char * b, char * cmd, int cmd_len)
{
  sock_pasv = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP);
  if(sock_pasv < 0)
  {
    console_print(RED "socket: %d %s\n" RESET, errno, strerror(errno));
    ftp_exit();
  ...
  console_print(GREEN "Successfully made PASV socket\n" RESET);

  cli_addr.sin_port        = htons(pasv_port);

  if (connect(sock_pasv, (struct sockaddr *) &cli_addr, sizeof(cli_addr)) < 0)
  {
    console_print(RED "Failed to connect with server\n" RESET);
    ftp_exit();
    return -1;
  }
  console_print(GREEN "Successfully connected to PASV port\n" RESET);

  simple_send(sock, cmd, cmd_len);
  simple_recv(sock, b);
  simple_recv(sock_pasv, b);
  close(sock_pasv);
  return 0;
}
...
int ftpc_ch_cmd()
{
  ...
  console_set_status(WHITE "%s\n" RESET, ftpc_commands[cur_ftpc_command].name);
  return 0;
}
...
int ftpc_do_cmd()
{
  if(sock == -1)
  {
    console_set_status(RED "Client not connected; socket not initialized\n" RESET);
    //console_print(RED "Client not connected; socket not initialized\n" RESET);
    return -1;
  }
  ftpc_commands[cur_ftpc_command].handler(NULL, NULL);
  return 0;
}
...
FTP_DECLARE(CWD)
{
  console_print(YELLOW "NOT IMPLEMENTED\n" RESET);
}
