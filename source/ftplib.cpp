...
int ftplib::readline(char *buf,int max,ftphandle *ctl)
{
	...
	do
	{
		...
		if (true)
		{
			...
			//console_print(WHITE "%s" RESET, ctl->cput);//FIXME
		}
...
int ftplib::readresp(char c, ftphandle *nControl)
{
	...
	//console_print(BLUE "write_resp: %s" RESET, nControl->response);//FIXME
	if (readline(nControl->response,256,nControl) == -1) //<<<<<<<<<
	{
		perror("Control socket read failed");
		console_print(RED "Control socket read failed 0\n" RESET);//FIXME
		return 0;
	}
	...
	if (nControl->response[3] == '-')
	{
		...
		do
		{
			...
			{
				perror("Control socket read failed");
				console_print(RED "Control socket read failed 1\n" RESET);//FIXME
				return 0;
			}
...
int ftplib::Connect(const char *host)
{
	...
	if (pnum == NULL)
	{
		if ((pse = getservbyname("ftp","tcp")) == NULL)
		{
			...
			console_print(RED "getservbyname\n" RESET);//FIXME
			free(lhost);
			return 0;
		}
...
#endif
	{
		if ((phe = gethostbyname(lhost)) == NULL)
		{
			...
			console_print(RED "gethostbyname\n" RESET);//FIXME
			free(lhost);
			return 0;
		}
...
int ftplib::FtpSendCmd(const char *cmd, char expresp, ftphandle *nControl)
{
	...
	console_print(BLUE "write: %s" RESET, buf);//FIXME
#endif
	if (x <= 0)
	{
		perror("write");
		console_print(RED "write\n" RESET);//FIXME
		return 0;
	}
...
int ftplib::FtpAccess_get(const char *path, accesstype type, transfermode mode, ftphandle *nControl, ftphandle **nData)
{
	char buf[256];
	int dir, ret;
...
int ftplib::FtpRead_dir(void *buf, int max, ftphandle *nData)
{
	int i;
...
int ftplib::FtpRead_nlst(void *buf, int max, ftphandle *nData,std::list<char *> * dir_lst)
{
	int i;
...
int ftplib::FtpXfer(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode)
{
	...
	if (localfile != NULL)
	{
		...
		//local = fopen(localfile, (type == ftplib::filewrite) ? "r" : "w");
		char ac[3] = "  ";
	...
	else
	{
		while ((l = FtpRead(dbuf, FTPLIB_BUFSIZ, nData)) > 0)
		{
			...
			else if (fwrite(dbuf, 1, l, local) <= 0)
			{
				...
				console_print(RED "localfile write\n" RESET);//FIXME
				break;
			}
		}
	}
...
int ftplib::FtpXfer_get(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode)
{
	int l,c;
  long wb;
  int mbw;
  int fsize = -1;
	...
	if (ftplib::Size(path, &fsize, ftplib::image) != 1){ 
    console_print(RED "ftplib::Size(path, &fsize, ftplib::image) != 1\n" RESET);//FIXME
    return 0;
  }
...
/*
  ...
  while(1)
  {
    ...
    {
      console_print(RED "wb(%d) != fsize(%d) RETRYING\n" RESET,wb,fsize);//FIXME
    }
    */
    console_print(CYAN "Transferring %uB\n" RESET,fsize);//FIXME
    wb = 0;
    mbw = 0;
    local = NULL;
    if (!FtpAccess_get(path, type, mode, nControl, &nData)){ 
      if (local != NULL) fclose(local);
      console_print(RED "!FtpAccess(path, type, mode, nControl, &nData))\n" RESET);//FIXME
      return 0;
    }
    while ((l = FtpRead(dbuf, FTPLIB_BUFSIZ, nData)) > 0)
    {
      if(local == NULL) local = fopen(localfile, "wb");
      ...
      wb += l;
      if(wb/FTPLIB_BUFSIZ > mbw)
      {
        console_set_status(CYAN "TRANSFERING: %u/%u MB written\n" RESET, wb/FTPLIB_BUFSIZ,fsize/FTPLIB_BUFSIZ);
        mbw = wb/FTPLIB_BUFSIZ;
      }
    }
    fclose(local);
  //}
    ...
    {
        console_print(GREEN "%uB written\n" RESET, wb);//FIXME
    }
	free(dbuf);
	return FtpClose(nData);
}

int ftplib::FtpXfer_dir(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode)
{
	int l,c;
	char *dbuf;
	FILE *local = NULL;
	...
	{
		while (l = FtpRead_dir(dbuf, FTPLIB_BUFSIZ, nData) > 0)
		{
			if(local == stdout) {
			}
			else if (fwrite(dbuf, 1, l, local) <= 0)
...
int ftplib::FtpXfer_nlst(const char *localfile, const char *path, ftphandle *nControl, accesstype type, transfermode mode, std::list<char *> * dir_lst)
{
	int l,c;
	char *dbuf;
	FILE *local = NULL;
	...
	{
		while (l = FtpRead_nlst(dbuf, FTPLIB_BUFSIZ, nData, dir_lst) > 0)
		{
			if(local == stdout) {
			}
			else if (fwrite(dbuf, 1, l, local) <= 0)
 ...
 * return 1 if successful, 0 otherwise
 */
int ftplib::Nlst(const char *outputfile, const char *path, std::list<char *> * dir_lst)
{
  int ret = 0;
  ...
  ret = FtpXfer_nlst(outputfile, path, mp_ftphandle, ftplib::dir, ftplib::ascii, dir_lst);
	return ret;
}
...
{
	mp_ftphandle->offset = 0;
	return FtpXfer_dir(outputfile, path, mp_ftphandle, ftplib::dirverbose, ftplib::ascii);
}
...
int ftplib::Get(const char *outputfile, const char *path, transfermode mode, off64_t offset)
{
	...
	if (offset == 0) return FtpXfer_get(outputfile, path, mp_ftphandle, ftplib::fileread, mode);
	else return FtpXfer_get(outputfile, path, mp_ftphandle, ftplib::filereadappend, mode);
}
